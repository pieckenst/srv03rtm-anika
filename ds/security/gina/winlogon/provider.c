    ...
    )
{
    ...
    if ( !VirtualLock( pNotifyData->Env, MemInfo.RegionSize ) ) {
        return FALSE ;
    }
    ...
    )
{
    ...
    HKEY Key2 = NULL;
    LPTSTR ProviderNameStart;
    DWORD ProviderNameSize;
    DWORD KeyNameSize;
    DWORD ProviderClass;
    DWORD cbProviderClass;
    BOOLEAN ProviderKeyOpened = FALSE;
    BOOLEAN Key2Opened = FALSE;
    ...
    while (*p) {
        ...
        ProviderNameSize = p - ProviderNameStart;
        if (ProviderNameSize == 0) {
            continue;
        }
        ProviderName = Alloc((ProviderNameSize + 1) * sizeof(TCHAR));
        if (ProviderName == NULL) {
            ErrorCode = ERROR_NOT_ENOUGH_MEMORY;
            goto Cleanup;
        }
        wcsncpy(ProviderName, ProviderNameStart, ProviderNameSize);
        ProviderName[ProviderNameSize] = 0;
        KeyNameSize = wcslen(TEXT("system\\CurrentControlSet\\Services\\"));
        KeyNameSize += ProviderNameSize;
        KeyNameSize += wcslen(TEXT("\\NetworkProvider"));
        KeyName = Alloc((KeyNameSize + 1) * sizeof(TCHAR));
        if (KeyName == NULL) {
            ErrorCode = ERROR_NOT_ENOUGH_MEMORY;
            goto Cleanup;
        }
        wcscpy(KeyName, TEXT("system\\CurrentControlSet\\Services\\"));
        wcscat(KeyName, ProviderName);
        wcscat(KeyName, TEXT("\\NetworkProvider"));
        ErrorCode = RegOpenKeyEx(
                    HKEY_LOCAL_MACHINE,     // hKey
                    KeyName,                // lpSubKey
                    0,                      // Must be 0
                    KEY_QUERY_VALUE,        // Desired access
                    &Key2                   // Newly Opened Key Handle
                    );
        if (ErrorCode != ERROR_SUCCESS) {
            goto Cleanup;
        }
        Key2Opened = TRUE;
        cbProviderClass = sizeof(ProviderClass);
        ErrorCode = RegQueryValueEx(Key2, TEXT("Class"), NULL, &ValueType, (LPBYTE)&ProviderClass, &cbProviderClass);
        if (ErrorCode != ERROR_SUCCESS) {
